	...
	public function actionDetailProvider()
			{
				...
				$this->renderPartial('provider_default',array(
						'model'=>$datas,'data'=> $data[0]
				),false,false);
			}
				...
				if($_FILES["fileUpload"]["error"]==0)
					...
					$subdomain = $_POST['websubmain'];
						if($id != ""){
							...
							$web->id_user = $user;
							$web->id_company = $id;
							mkdir(Yii::getPathOfAlias('webroot')."/protected/uploads/mini-websites/".$subdomain);
							$web->insert();
							$miniweb = $web->id;
								if($miniweb !="")
								{
...
public function actionSearchProvider()
{
	...
	if($_POST['type'] == 4){
		$orderBy = 'a.`code_number` DESC ';
	}

	if ($_POST['value']) 
	{
		$search_params= 'AND (a.`Name` LIKE "%'.$_POST['value'].'%" )';
	}
