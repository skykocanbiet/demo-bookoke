...
class DealselitedentalController extends Controller
{
	...
	public function actionAddDeals(){
		$name =  $_POST['dealsName'];
		$data = Promotion::model()->findAllByAttributes(array("name"=>$name));
		if(count($data) > 0){
			echo 0;
			exit();
		}else{ 
		...
		$user = Yii::app()->user->getState('user_id');
				...
				elseif ($_POST['type_price'] == 2){
					$model->price =  preg_replace('([^a-zA-Z0-9])', '',  $_POST['value_promotion2']);
				}
				elseif ($_POST['type_price'] == 3){
					$model->price =  preg_replace('([^a-zA-Z0-9])', '',  $_POST['value_promotion3']);
				}
	...
	public function actionPrice(){
		if(isset($_POST['value']) && $_POST['value'] != ''){
			//$model = new Product;
			$data = CsService::model()->findAllByAttributes(array("id"=>$_POST['value']));
			echo $data[0]['price'];
			//print_r($data);
			//echo implode('', $data);
		}
	...
	public function actionPriceproduct(){
		if(isset($_POST['value']) && $_POST['value'] != ''){
			//$model = new Product;
			$data = Product::model()->findAllByAttributes(array("id"=>$_POST['value']));
			echo $data[0]['price'];
			//print_r($data);
			//echo implode('', $data);
		}
...
public function actionupdategroup(){
	if (isset($_POST['id']) ) {
	$name = $_POST['txtname'];	# code...
	$id = $_POST['id'];
	CroupPromotion::model()->updateByPk($id, array('name'=>$name));
	echo "1";
	exit();
	}
	echo "0";
