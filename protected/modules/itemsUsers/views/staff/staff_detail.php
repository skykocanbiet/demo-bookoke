...
<div id="content_tab_information" style="overflow: auto;" class="col-sm-12">
	<form method="post" role="form" id="form_data_user" name="form_data_user" enctype="multipart/form-data" class="row" style="margin-top: 30px">
		<input type="hidden" id="id_dentist" name="id_dentist" value="<?php echo $model->id; ?>"></input>
		<input type="hidden" id="codeNumber_user" name="codeNumber_user" value="<?php echo $model->code; ?>"></input>
		...
		<div class="col-sm-12" style="margin-bottom: 30px">
			...
			<div class="col-sm-10" style="margin: 13px auto;">
			...
			<div class="col-sm-2">
				<!-- <input id="btn_save_user" value="Lưu Lại" type="button" name="btn_save_user" style="width: 70px;height: 35px;background: #10b1dd;color: #fff;border-radius: 8px;padding: 3px 8px;font-size: 15px;border: solid 1px #D7D7D7;">
				</input> -->
			</div>
		</div>
		<div class="col-sm-6">
			<div class="col-sm-12" id="rowHS">
				<div class="col-sm-3 lable">
			...
			<div class="col-sm-12 password" id="rowHS">
				...
				<div class="col-sm-9">
					<input class="input-form" disabled type="password" id="password" name="password" value='<?php echo $model['username']; ?>'><br>
					<span onclick="change_pass()" style="color: #6ec4a1;margin-top: 10px;cursor: pointer;margin-left: 4px;">Đổi mật khẩu</span>
				</div>
			</div>
			...
			<div class="col-sm-12" id="rowHS">
				...
				<div class="col-sm-9">
					<input class="input-form" type="text" id="phone" name="phone" value='<?php echo $model->phone ?>'>
				</div>
			</div>
			<div class="col-sm-12" id="rowHS">
				<div class="col-sm-3 lable">
		...
		<div class="col-sm-6">
			<div class="col-sm-12" id="rowHS">
				...
				<div class="col-sm-9">
					<select class="input-form" id="branch_user" name="branch_user">
						<?php foreach ($branch_user as $value) {
						if($model['id_branch'] == $value['id']){
					?>
			...
			<div class="col-sm-12" id="rowHS">
				...
				<div class="col-sm-9">
					<select <?php echo(Yii::app()->user->getState('group_id') ==3)?'disabled="disabled"':""; ?> class="input-form" id="group_user" name="group_user">
		...
		<div class="col-sm-12" id="rowHS" style="padding-left: 30px">
			<hr style="margin: 40px auto;" >
			<div class="col-sm-2 lable">
				<label>Kinh Nghiệm:</label>
			</div>
			<div class="col-sm-10">
				<textarea style="height: 75px; overflow: hidden; word-wrap: break-word; resize: none;" id="staff_exp" name="staff_exp" class="span7 yellow_hover blue_focus input-form" placeholder="Staff Description"><?php echo $model['exp']; ?></textarea>
			</div>
		</div>
		<div class="col-sm-12" id="rowHS" style="padding-left: 30px">
			<div class="col-sm-2 lable">
			...
			<div class="col-sm-10">
				<textarea style="height: 75px; overflow: hidden; word-wrap: break-word; resize: none;" id="staff_diploma" name="staff_diploma" class="span7 yellow_hover blue_focus input-form" placeholder="Staff Description"><?php echo $model['diploma']; ?></textarea>
			</div>
		</div>
		<div class="col-sm-12" id="rowHS" style="padding-left: 30px">
			<div class="col-sm-2 lable">
			...
			<div class="col-sm-10">
				<textarea style="height: 75px; overflow: hidden; word-wrap: break-word; resize: none;" id="staff_certificate" name="staff_certificate" class="span7 yellow_hover blue_focus input-form" placeholder="Staff Description"><?php echo $model['certificate']; ?></textarea>
			</div>
		</div>
	</form>
</div>
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#confirm_pass,#pass_old').change(function(){
			...
			jQuery.ajax({
	            ...
	            success:function(data)
	            {
	            	...
	            	if(data == 0)
	            	{
	            		$.jAlert({
		                    'title': "Thông báo !",
		                    'content': "Mật khẩu cũ không đúng! Vui lòng nhập lại"
		                });
	            		...
	            		if(pass_new =="")
							...
							else if(pass_new != confirm_pass)
							{
								$.jAlert({
				                    'title': "Thông báo !",
				                    'content': "Xác nhận lại mật khẩu không khớp ! Vui lòng kiểm tra lại."
				                });
		...
		$('#staff_exp,#staff_diploma,#staff_certificate,#example-enableClickableOptGroups-disabled,#block_user,#group_user,#branch_user,#name_user,#username,#phone').change(function(){
			saveinfo();
		});
	...
	function UpdateImageUser(){
			var formData = new FormData($("#form_data_user")[0]);
			 jQuery.ajax({
	            type: "POST",
	            url: "<?php echo CController::createUrl('Staff/UpdateImageUser')?>",
	            data: formData,
		        cache: false,
		        processData: false,
		        contentType: false,
	            ...
	            {         
	            	if(data)
	            	{
	            		var id_user 	= $("#id_dentist").val();
	            		$(".user_img"+id_user).attr("src","/upload/users/sm/"+data);

	            		$('#alert-success').append('Cap nhat thanh cong');
	            		var element = $('.alert-success');
	            		// var count = element.length;
	            		for (var i = element.length; i >= 0; i--) {
	            			$(element[i]).fadeTo(2000, 500).slideUp(500, function(){
