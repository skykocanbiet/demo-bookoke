...
<form class="form-inline">
    ...
    <div id="oSrchLeft" class="pull-right" style="text-align: right;">
        <!-- Split button -->
        <div class="btn-group">
          <button type="button" class="btn btn_bookoke" onclick="detailreportservice()"><i class="fa fa-search-plus"></i>&nbsp;Xem</button>
...
<div id="select_group_service" class="form-group col-sm-3" style=" margin-top: 10px; margin-left: -15px;" >
                <label style="margin-top: 7px;">Nhóm dịch vụ: </label>
              <?php 
...
*/                    $User         = CsServiceType::model()->findAllByAttributes(array('status'=>1));
                    ...
                    echo CHtml::dropDownList('frm_search_nhom_dich_vu',3,$listdata,array('onChange'=>"search_service(this.value);",'class'=>'Service form-control'));
                    
                ?>
                <input style="display: none;" type="text" value="3" id="frm_search_requester_date" name="frm_search_received_date" onkeypress="runScript(event);" style="width: 160px;"/>
 </div> 
 <div id="select_service" class="form-group col-sm-3" style=" margin-top: 10px; margin-left: -15px;" >
                <label style="margin-top: 7px;">Chọn dịch vụ: </label>
                <span id="service">
...
<div class="hide col-sm-6" id="date" style=" margin-top: 10px;">
    <div class="form-group" style="margin-right:20px;">  
...
<script>
 function search_date(cur_page)
    ...
    $('.cal-loading').fadeIn('fast');     
        ...
        var id_user         = $("#frm_search_user_id").val();
        var location        = $("#frm_search_branch").val();
        var group 			= $('#frm_search_nhom_dich_vu').val();
        var service         = $('#frm_search_dich_vu').val();
    $.ajax({
        type:'POST',
        url: "<?php echo CController::createUrl('ReportingCustomer/DetailReportservice')?>", 
        data: {             time        : time,
                            id_user     : id_user,
                            location    : location,
                            date        : date,
...
$( document ).ready(function() {
    ...
    $('#oSrchBar').on('click','.print',function(e){
        ...
        var group 			= $('#frm_search_nhom_dich_vu').val();
        var service         = $('#frm_search_dich_vu').val();
        var url="<?php echo CController::createUrl('ReportingCustomer/printreport')?>?type="+type+"&location="+location+"&id_user="+id_user+"&time="+time+"&date_start="+date_start+"&date_end="+date_end+"&group="+group+"&service="+service;
...
function search_service(id){
	 $.ajax({
        type:'POST',
        url: "<?php echo CController::createUrl('ReportingCustomer/GetService')?>", 
        data: {             id : id
                        },   
        success:function(data){     
