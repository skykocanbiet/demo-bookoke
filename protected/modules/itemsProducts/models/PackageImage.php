 ...
 *
 * The followings are the available columns in table 'package_image':
 * @property integer $id
 ...
 */
class PackageImage extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_package', 'required'),
			array('id_package, kind, size, status', 'numerical', 'integerOnly'=>true),
			...
			array('id, id_package, name, url_action, name_upload, update_time, kind, size, status', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return PackageImage the static model class
	 */
    ...
    public function deleteImageScaleAndCrop($fileImageDelete){
    	   ...
    	   unlink("upload/package_image/lg/".$fileImageDelete);
		   unlink("upload/package_image/md/".$fileImageDelete);
		   unlink("upload/package_image/sm/".$fileImageDelete);
    }
}
